...
def convert_weights(weights, dump_weights):
    ...
    weights["vision_patch_embedding_w"].copy_(
        torch.tensor(
            dump_weights["PaliGemma"]["img"]["embedding"]["kernel"]["value"], dtype=torch.bfloat16, device="cpu"
        )
    )
    weights["vision_patch_embedding_b"].copy_(
        torch.tensor(dump_weights["PaliGemma"]["img"]["embedding"]["bias"]["value"], dtype=torch.bfloat16, device="cpu")
    )
    weights["vision_position_embedding"].copy_(
        torch.tensor(
            dump_weights["PaliGemma"]["img"]["pos_embedding"]["value"], dtype=torch.bfloat16, device="cpu"
        ).squeeze()
    )
    ...
    weights["vision_ffn_up_w"].copy_(
        torch.tensor(
            dump_weights["PaliGemma"]["img"]["Transformer"]["encoderblock"]["MlpBlock_0"]["Dense_0"]["kernel"]["value"],
            dtype=torch.bfloat16,
            device="cpu",
        )
    )
    weights["vision_ffn_up_b"].copy_(
        torch.tensor(
            dump_weights["PaliGemma"]["img"]["Transformer"]["encoderblock"]["MlpBlock_0"]["Dense_0"]["bias"]["value"],
            dtype=torch.bfloat16,
            device="cpu",
        )
    )
    weights["vision_ffn_down_w"].copy_(
        torch.tensor(
            dump_weights["PaliGemma"]["img"]["Transformer"]["encoderblock"]["MlpBlock_0"]["Dense_1"]["kernel"]["value"],
            dtype=torch.bfloat16,
            device="cpu",
        )
    )
    weights["vision_ffn_down_b"].copy_(
        torch.tensor(
            dump_weights["PaliGemma"]["img"]["Transformer"]["encoderblock"]["MlpBlock_0"]["Dense_1"]["bias"]["value"],
            dtype=torch.bfloat16,
            device="cpu",
        )
    )
    ...
    weights["vision_pre_attn_norm_w"].copy_(
        torch.tensor(
            dump_weights["PaliGemma"]["img"]["Transformer"]["encoderblock"]["LayerNorm_0"]["scale"]["value"],
            dtype=torch.bfloat16,
            device="cpu",
        )
    )
    weights["vision_pre_attn_norm_b"].copy_(
        torch.tensor(
            dump_weights["PaliGemma"]["img"]["Transformer"]["encoderblock"]["LayerNorm_0"]["bias"]["value"],
            dtype=torch.bfloat16,
            device="cpu",
        )
    )
    weights["vision_pre_ffn_norm_w"].copy_(
        torch.tensor(
            dump_weights["PaliGemma"]["img"]["Transformer"]["encoderblock"]["LayerNorm_1"]["scale"]["value"],
            dtype=torch.bfloat16,
            device="cpu",
        )
    )
    weights["vision_pre_ffn_norm_b"].copy_(
        torch.tensor(
            dump_weights["PaliGemma"]["img"]["Transformer"]["encoderblock"]["LayerNorm_1"]["bias"]["value"],
            dtype=torch.bfloat16,
            device="cpu",
        )
    )
    weights["vision_final_norm_w"].copy_(
        torch.tensor(
            dump_weights["PaliGemma"]["img"]["Transformer"]["encoder_norm"]["scale"]["value"],
            dtype=torch.bfloat16,
            device="cpu",
        )
    )
    weights["vision_final_norm_b"].copy_(
        torch.tensor(
            dump_weights["PaliGemma"]["img"]["Transformer"]["encoder_norm"]["bias"]["value"],
            dtype=torch.bfloat16,
            device="cpu",
        )
    )
    ...
    weights["encoder_multi_modal_projector_w"].copy_(
        torch.tensor(dump_weights["PaliGemma"]["img"]["head"]["kernel"]["value"], dtype=torch.bfloat16, device="cpu")
    )
    weights["encoder_multi_modal_projector_b"].copy_(
        torch.tensor(dump_weights["PaliGemma"]["img"]["head"]["bias"]["value"], dtype=torch.bfloat16, device="cpu")
    )
    ...
    w_attn_o = (
        dump_weights["PaliGemma"]["llm"]["layers"]["attn"]["attn_vec_einsum"]["w"]["value"]
    ...
    w_attn_o = (
        dump_weights["PaliGemma"]["llm"]["layers"]["attn"]["attn_vec_einsum_1"]["w"]["value"]
    ...
    weights["decoder_state_in_proj_w"].copy_(
        torch.tensor(dump_weights["state_proj"]["kernel"]["value"], dtype=torch.bfloat16, device="cpu")
    )
    weights["decoder_state_in_proj_b"].copy_(
        torch.tensor(dump_weights["state_proj"]["bias"]["value"], dtype=torch.bfloat16, device="cpu")
    )
    ...
    mlp_in_weight_action = torch.tensor(
        dump_weights["action_time_mlp_in"]["kernel"]["value"][:1024, :], dtype=torch.bfloat16, device="cpu"
    )
    mlp_in_weight_time = torch.tensor(
        dump_weights["action_time_mlp_in"]["kernel"]["value"][1024:, :], dtype=torch.bfloat16, device="cpu"
    )
    action_time_mlp_in_b = torch.tensor(
        dump_weights["action_time_mlp_in"]["bias"]["value"], dtype=torch.bfloat16, device="cpu"
    )
    action_in_proj_w = torch.tensor(
        dump_weights["action_in_proj"]["kernel"]["value"], dtype=torch.bfloat16, device="cpu"
    )
    action_in_proj_b = torch.tensor(dump_weights["action_in_proj"]["bias"]["value"], dtype=torch.bfloat16, device="cpu")
    decoder_action_fused_out_proj_w = torch.tensor(
        dump_weights["action_out_proj"]["kernel"]["value"], dtype=torch.bfloat16, device="cpu"
    )
    decoder_action_fused_out_proj_b = torch.tensor(
        dump_weights["action_out_proj"]["bias"]["value"], dtype=torch.bfloat16, device="cpu"
    )
    final_norm_scale = torch.tensor(
        dump_weights["PaliGemma"]["llm"]["final_norm_1"]["scale"]["value"], dtype=torch.bfloat16, device="cpu"
    )
    ...
    weights["decoder_action_mlp_w"].copy_(
        torch.tensor(dump_weights["action_time_mlp_out"]["kernel"]["value"], dtype=torch.bfloat16, device="cpu")
    )
    weights["decoder_action_mlp_b"].copy_(
        torch.tensor(dump_weights["action_time_mlp_out"]["bias"]["value"], dtype=torch.bfloat16, device="cpu")
    )
